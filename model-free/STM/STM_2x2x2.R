...
# ===============================================================================
# 2×2×2×Load MIXED ANOVA (Condition × GCS × SSS × Load)
# ===============================================================================

cat("=== 2×2×2×Load Mixed ANOVA (Condition × GCS × SSS × Load) ===\n\n")

# Convert to long format: Colour and Location become levels of "condition"
stm_colour_long <- combined_averages %>%
  mutate(
    condition = "Colour",
    deviation = avg_colour_angle_abs_deviation
  ) %>%
  select(participant_id, subtype_group, has_GCS, has_SSS, load_n, condition, deviation)

stm_location_long <- combined_averages %>%
  mutate(
    condition = "Location",
...
stm_long <- bind_rows(stm_colour_long, stm_location_long) %>%
  mutate(
    condition = factor(condition),
    has_GCS = factor(has_GCS),
    has_SSS = factor(has_SSS),
    load_n = factor(load_n)
  )

# Mixed ANOVA with repeated measures on condition
stm_anova <- aov(
  deviation ~ condition * has_GCS * has_SSS * load_n +
    Error(participant_id / condition),
  data = stm_long
)

print(summary(stm_anova))
